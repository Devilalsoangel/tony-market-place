...
export default function CommissionPage() {
  ...
  const { data: promoRows } = useDbResource<PromotionPurchase>("promotions");
  const { data: orderRows } = useDbResource<Order>("orders");
  const { data: withdrawalRows } = useDbResource<WithdrawalRequest>("withdrawals");
  ...
  const deliveredGmv = (orderRows ?? []).filter((o) => o.status === "delivered").reduce((s, o) => s + o.amount, 0);
  const commissionOnDelivered = Math.round((deliveredGmv * rate) / 100);
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-0.5 text-[13px] text-[#71717A]">
          ...
          seller wallets reflect — the default rate applies to every delivered order.
        </p>
      </div>
      ...
      <div className="grid grid-cols-4 gap-4">
        ...
        <StatTile label={`Promo revenue (${activePromos.length} active)`} value={inr(promoRevenue)} tone="purple" />
        <StatTile label={`Payout fees (${settledPayouts.length} payouts)`} value={inr(payoutFees)} />
      </div>
